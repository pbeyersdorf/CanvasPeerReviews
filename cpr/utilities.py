...
def createRelatedAssignment(assignment, separateGroup=True):
	...
	#the problem with this is 'assignment' is not the peer review assignment, but the one it is based off ot
		...
		print("Error accessing the 'peer review assignment description' template, using default language instead: \n{description}")
	creationDict={
	'name': assignmentName,
...
def getParameters(ignoreFile=False, selectedAssignment="all"):
	...
	if not params.loadedFromFile or ignoreFile:
		...
		if params.combineSubmissionAndReviewGrades:
			...
			if not course.apply_assignment_group_weights:
				confirm(f"\033[1m"+"Make sure to set 'Weight final grade based on assignment groups' in canvas (under the assignmetns tab) for these weights to take effect"+"\033[0m",False)
	...
	if (not params.loadedFromFile or ignoreFile) and confirm("Shall any changes be saved? "):
		saveData()
	return params
